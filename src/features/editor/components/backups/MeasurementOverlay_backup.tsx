...
const BUILDING_TYPES = new Set<ObjectType>([
    "house",
    "garage",
    "shed",
    "garden_house",
...
function buildAreaLabelRenderData(
    ...
    if (object.type === "plantbed") {
        const layout = plantbedNumberLayouts.get(object.id);
        if (!layout) return null;

        if (layout.areaRotation === 0) {
            const textWidth = estimateTextWidth(layout.areaText, layout.areaFontSize);
...
function DraftSegmentMeasurements({
    draftPoints,
    stageScale,
}: {
    draftPoints: number[];
    ...
    const lastIndex = draftPoints.length / 2 - 2;

    if (lastIndex < 0) return null;

    const ax = draftPoints[lastIndex * 2];
    const ay = draftPoints[lastIndex * 2 + 1];
    const bx = draftPoints[(lastIndex + 1) * 2];
    const by = draftPoints[(lastIndex + 1) * 2 + 1];
    ...
    return (
        <OrangePillLabel
            key={`draft-segment-measure-${lastIndex}`}
    ...
    return (
        <Group listening={false}>
            ...
            {showSelectedDimensions && selectedObjects.length === 1 && primarySelectedObject ? (
                <SelectedObjectDimensions object={primarySelectedObject} stageScale={stageScale} />
            ) : null}

            {shouldRenderDraftMeasurements ? (
                <DraftSegmentMeasurements draftPoints={draftMeasurementPoints} stageScale={stageScale} />
            ) : null}
        </Group>
    );
