...
export const OBJECT_STYLES: Record<ObjectType, { fill: string; stroke: string }> = {
    grass: { fill: "#DCE9DC", stroke: "#4F6B4F" },
    tiles: { fill: "#E6E6E6", stroke: "#8A8A8A" },
    water: { fill: "#DCEAF4", stroke: "#5C89A6" },
    gravel: { fill: "#CFC6B6", stroke: "#9C8F7A" },
    sand: { fill: "#F3E2A4", stroke: "#C2A44A" },
    wood: { fill: "#C9A27A", stroke: "#8B5E3C" },
    patio: { fill: "#E2D4C2", stroke: "#B8A48C" },

    house: { fill: "#D9A08F", stroke: "#8C3A2F" },
    garage: { fill: "#C88F80", stroke: "#7C3A33" },
    shed: { fill: "#A87867", stroke: "#4A2B24" },
    garden_house: { fill: "#D7C19A", stroke: "#6B4E2E" },
    carport: { fill: "#D8D8D8", stroke: "#6F6F6F" },
    veranda: { fill: "#E7C6BA", stroke: "#8C5A45" },
    canopy: { fill: "#EFE3DC", stroke: "#7A6A5A" },

    fence: { fill: "#A89173", stroke: "#8D6819" },
    gate: { fill: "#D8BB96", stroke: "#A89173" },

    plantbed: { fill: "#F2FDEF", stroke: "#3F6B3F" },
    treebed: { fill: "#008000", stroke: "#008000" },
};

export const TYPE_Z_INDEX: Record<ObjectType, number> = {
    // Ondergrond
    grass: 1,
    sand: 2,
    gravel: 3,
    ...
    // Beplanting (onder gebouwen)
    plantbed: 20,
    treebed: 42,

    // Gebouwen (boven beplanting)
    house: 30,
    garage: 31,
    shed: 32,
    garden_house: 33,
    ...
    // Erfafscheiding (boven alles behalve labels later)
    gate: 40,
    fence: 41,
};

export type EditorTool = "select" | "hand" | "cut" | "draw";
...
// ✅ Geometry helpers
// -----------------------------
const LINE_TYPES: ObjectType[] = ["fence", "gate"];
...
function getGeometryForType(t: ObjectType) {
    return isLineType(t) ? ("polyline" as const) : ("polygon" as const);
}
...
const SNAP_GRID_SIZE = 25; // zelfde als editor GRID_SIZE (later evt. centraliseren)
...
function renumberPlantbedsSequential(objects: PolyObject[]) {
    ...
    return objects.map((obj) => {
        ...
        const currentNo = obj.plantbedNo;

        if (
            Number.isFinite(currentNo) &&
            (currentNo as number) > 0 &&
            !used.has(currentNo as number)
        ) {
            used.add(currentNo as number);
            return obj;
        }

        return {
            ...obj,
            plantbedNo: takeNextFree(),
        };
    });
}
...
export const useProjectStore = create<ProjectState>((set, get) => ({
    ...
    linkPlantToPlantbed: (plantId, plantbedId) =>
        set((state) => {
            ...
            const { plantbedLinks, plantbedLinkedCount } = applyLink(state as ProjectState, plantId, plantbedId);
    ...
    getPolylineRenderPieces: (points, type) => {
        ...
        if (!(type === "fence" || type === "gate")) return [];

        const draftObj: PolyObject = {
            id: "__draft_polyline__",
            type,
            points,
            geometry: "polyline",
        } as any;

        const world = [...get().objects, draftObj] as PolyObject[];
        return computeLineRenderPieces(draftObj, world);
    },
    ...
    setObjectsWithHistory: (nextObjects, nextSelectionId = null) =>
        set((state) => {
            // ✅ Normaliseer geometries + bereken renderPieces voor fence/gate direct bij commit
            const normalized = (nextObjects ?? []).map((o) => ({
                ...o,
                geometry: (o as any).geometry ?? getGeometryForType((o as any).type),
            })) as PolyObject[];

            const lineObjects = normalized.filter((o) => isPolylineObject(o));
            const reconciled = reconcilePolygonsAgainstLineObjects(normalized, lineObjects);

            // world waartegen we clippen = de nieuwe set zelf
            const world = reconciled;

            // ✅ renderPieces vullen voor alle polyline objects
            const withPieces = world.map((o) => {
                if (!isPolylineObject(o)) return o;
                return withLinePieces({ ...o, geometry: "polyline" }, world);
            });
    ...
    addObject: (obj) =>
        ...
        set((state) => {
            ...
            // ✅ LINE OBJECTS (fence/gate): NIET door polygon pipeline
            if (isPolylineObject(newObj)) {
                const id = newObj.id ?? makeId();

                const baseObj: PolyObject = { ...newObj, id, geometry: "polyline" };

                const reconciledExisting = reconcilePolygonsAgainstLineObjects(
                    state.objects as PolyObject[],
                    [baseObj]
                );
    ...
    moveObjectAndMerge: (id, toPoints, toHoles) =>
        set((state) => {
            ...
            // ✅ LINE OBJECTS (fence/gate): update punten + reconcile aangrenzende polygonen
            ...
            if (type === "plantbed") {
                ...
                const movedPlantbeds: PolyObject[] = movedShapes
                    .filter((pts) => pts.length >= 6)
                    .map((pts: number[], idx: number) => {
                        const idToUse = idx === 0 ? existing.id : makeId();
            ...
            const movedObjectsRaw: PolyObject[] = movedShapes
                .filter((pts) => pts.length >= 6)
                .flatMap((pts, idx) =>
                    normalizeSingleObjectToPieces({
                        ...
                        // Belangrijk:
                        // bij edge-resize/self-normalisatie van één shape willen we de nieuwe contour
                        // opnieuw als geheel interpreteren. Oude holes van het bestaande object hier
                        // blind meenemen kan de topologie vervuilen en leidt juist tot terugspringen.
                        holes: undefined,
            ...
            if (movedObjectsRaw.length > 0) {
                ...
                if (candidates.length > 0) {
                    ...
                    console.log("[DEBUG merge route moveObjectAndMerge]", {
                        existingId: existing.id,
                        type,
                        hasNonOrthogonalHigher,
                        useCellMerge,
                        mergeInput: mergeInput.map((o) => ({
                            id: o.id,
                            type: o.type,
                            points: o.points,
                            holes: o.holes ?? [],
                        })),
                        relevantHigher: relevantHigher.map((o) => ({
                            id: o.id,
                            type: o.type,
                            points: o.points,
                            holes: o.holes ?? [],
