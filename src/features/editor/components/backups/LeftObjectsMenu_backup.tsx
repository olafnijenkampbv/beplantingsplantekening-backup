...
type Section = {
  id: "ondergrond" | "gebouwen" | "afbakening" | "beplanting";
  label: string;
  items: Item[];
};
...
const TREEBED_VARIANTS: Array<{ key: TreebedVariant; label: string }> = [
  ...
  return (
    ...
    >
                  ...
                  <div className="pb-2">
                    {sec.items.map((it) => {
                      ...
                      const swatchFill =
                        it.id === "treebed"
                          ? (() => {
                            const color = OBJECT_STYLES[it.id].fill;

                            if (color.startsWith("rgba(")) {
                              return color.replace(/rgba\(([^)]+),\s*[\d.]+\)/, "rgba($1, 0.6)");
                            }
      ...
      {isTreebedPanelMounted && treebedPanelPos && (
        <div
          ref={treebedPanelRef}
          className="fixed z-[80] w-[196px] rounded-md border transition-all duration-150 overflow-hidden"
          style={{
        ...
        >
          ...
          <div className="p-2 flex flex-col gap-0.5">
            {TREEBED_VARIANTS.map((item) => {
              const active = props.activeTreebedVariant === item.key;
              ...
              return (
                ...
                >
                  <span
                    ...
                    style={{
                      backgroundColor: "rgba(88, 105, 76, 0.85)",
                      borderColor: COLORS.green,
                    }}
                  />
                  <span className="text-[13px] leading-[1.2] text-black">{item.label}</span>
                </button>
              );
            })}
          </div>
