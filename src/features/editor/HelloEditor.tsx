...
import {
    ...
    mergeFenceOrGateEndpoints as mergeBoundaryFenceOrGateEndpoints,
} from "@/features/editor/lib/boundarySystem";
import {
...
type CachedPlantbedLayoutEntry = {
    signature: string;
    layout: PlantbedNumberLayout;
};

function roundLayoutSignatureNumber(value: number) {
    if (!Number.isFinite(value)) return 0;
    return Math.round(value * 100) / 100;
}

function buildFlatPointsSignature(points: number[]) {
    if (!points || points.length === 0) return "";

    return points
        .map((value) => String(roundLayoutSignatureNumber(value)))
        .join(",");
}

function buildHolesSignature(holes: number[][] = []) {
    if (!holes || holes.length === 0) return "";
    return holes.map((hole) => buildFlatPointsSignature(hole)).join("|");
}

function buildTreebedBlockersSignature(blockers: TreebedLabelBlocker[]) {
    if (!blockers || blockers.length === 0) return "";

    return blockers
        .map((blocker) =>
            [
                roundLayoutSignatureNumber(blocker.x),
                roundLayoutSignatureNumber(blocker.y),
                roundLayoutSignatureNumber(blocker.w),
                roundLayoutSignatureNumber(blocker.h),
            ].join(",")
        )
        .join("|");
}

function buildPlantbedLayoutSignature(
    points: number[],
    holes: number[][],
...
function getLiveHolePlantbedNumberLayout(
    points: number[],
    holes: number[][] = [],
    plantbedNo: number | string,
    areaText: string,
    ...
    const numberFontSize =
        baseLayout?.fontSize ??
        Math.round(clamp(Math.min(safeHeight * 0.32, safeWidth / Math.max(1, text.length * 0.72)), 4, 28));
    ...
    const compositeWidth = Math.max(numberTextWidth, areaTextWidth) + INNER_MARGIN * 2;
    const compositeHeight = numberTextHeight + AREA_GAP + areaTextHeight + INNER_MARGIN * 2;

    const candidateCenters = [
        { x: bb.x + bb.w / 2, y: bb.y + bb.h / 2 },
        { x: bb.x + bb.w * 0.6, y: bb.y + bb.h * 0.6 },
        { x: bb.x + bb.w * 0.4, y: bb.y + bb.h * 0.6 },
        { x: bb.x + bb.w * 0.6, y: bb.y + bb.h * 0.4 },
        { x: bb.x + bb.w * 0.4, y: bb.y + bb.h * 0.4 },
        { x: bb.x + bb.w * 0.5, y: bb.y + bb.h * 0.7 },
    ];

    const rectFitsInUsableArea = (centerX: number, centerY: number) => {
        const rect = {
            x: centerX - compositeWidth / 2,
            y: centerY - compositeHeight / 2,
            w: compositeWidth,
            h: compositeHeight,
        };
        ...
        return testPoints.every((point) => {
            if (!pointInPolygon(point.x, point.y, points)) return false;
            return !holes.some((hole) => pointInPolygon(point.x, point.y, hole));
        });
    };

    const chosenCenter =
        candidateCenters.find((candidate) => rectFitsInUsableArea(candidate.x, candidate.y)) ??
        { x: bb.x + bb.w / 2, y: bb.y + bb.h / 2 };

    const compositeTopY = chosenCenter.y - compositeHeight / 2 + INNER_MARGIN;
    const numberY = compositeTopY;
    const numberX = chosenCenter.x - numberTextWidth / 2;
    const areaY = compositeTopY + numberTextHeight + AREA_GAP;
    const areaX = chosenCenter.x - areaTextWidth / 2;
...
function getFastPlantbedNumberLayout(
    points: number[],
    holes: number[][] = [],
    plantbedNo: number | string,
    areaText: string,
    ...
    const centerCandidates = [
        { x: bb.x + bb.w / 2, y: bb.y + bb.h / 2 },
        { x: bb.x + bb.w * 0.65, y: bb.y + bb.h * 0.5 },
        { x: bb.x + bb.w * 0.35, y: bb.y + bb.h * 0.5 },
        { x: bb.x + bb.w * 0.5, y: bb.y + bb.h * 0.35 },
        { x: bb.x + bb.w * 0.5, y: bb.y + bb.h * 0.65 },
    ];
    ...
    const chosenCenter =
        centerCandidates.find((candidate) => isInsideUsableArea(candidate.x, candidate.y)) ??
        { x: bb.x + bb.w / 2, y: bb.y + bb.h / 2 };

    const compositeTopY =
        chosenCenter.y - (numberTextHeight + AREA_GAP + areaFontSize) / 2;

    return {
        text,
        fontSize: numberFontSize,
        x: chosenCenter.x - numberTextWidth / 2,
        y: compositeTopY,
        width: numberTextWidth,
        areaText,
        areaFontSize,
        areaRotation: 0,
        areaX: chosenCenter.x - areaTextWidth / 2,
        areaY: compositeTopY + numberTextHeight + AREA_GAP,
    };
}
...
function snapWorldPointAgainstFenceBoundary(
    ...
    if (targetType && isUnifiedBoundaryType(targetType)) {
        return getBoundarySnapPoint(rawX, rawY, GRID_SIZE);
    }
...
function resolveDrawPreviewPoint(
    ...
    if (shiftKey) {
        if (startX !== null && startY !== null) {
            const rawDistanceToStart = Math.hypot(rawX - startX, rawY - startY);

            if (rawDistanceToStart <= ORTHO_GUIDE_TOLERANCE) {
                return {
                    x: startX,
                    y: startY,
                    primaryGuidePoints: showGuides ? [anchorX, anchorY, startX, startY] : null,
                    secondaryGuidePoints: null,
                };
            }
        }
...
function getOneSidedPolylineRenderPoints(
    ...
    return getBoundaryOneSidedPolylineRenderPoints(points, strokeWidth, side);
}
...
function mergeFenceOrGateEndpoints(
    ...
    return mergeBoundaryFenceOrGateEndpoints(type, newPoints, objects);
}
...
export default function HelloEditor() {
    ...
    const rightStep2 = useRightStepMenuStore((s) => s.step2);
    ...
    const exportRightStepMenuSnapshotFromStore = useCallback((): PersistedRightStepMenuSnapshot => {
        ...
        return {
            ...
            step1: {
                locationType: state.step1.locationType,
                gardenZones: [...state.step1.gardenZones],
            },
            step2: {
                standplaatsen: [...state.step2.standplaatsen],
                groundTypes: [...state.step2.groundTypes],
                maintenanceLevel: state.step2.maintenanceLevel,
                certificationPreference: state.step2.certificationPreference,
            },
            step3: {
                structureStyle: state.step3.structureStyle,
                customPercentages: {
                    bodembedekkers: state.step3.customPercentages.bodembedekkers,
                    vastePlanten: state.step3.customPercentages.vastePlanten,
                    heestersEnStruiken: state.step3.customPercentages.heestersEnStruiken,
                    bomen: state.step3.customPercentages.bomen,
                },
            },
            step4: {
                seasonExperience: state.step4.seasonExperience,
    ...
    const plantbedNumberLayouts = useMemo(() => {
        ...
        for (const obj of numberedObjects) {
            ...
            if (cached?.signature === signature) {
                layouts.set(obj.id, cached.layout);
                validIds.add(obj.id);
                continue;
            }

            const layout = getPlantbedNumberLayout(
                obj.points,
                obj.holes ?? [],
                obj.plantbedNo ?? 0,
                areaText,
    ...
    const commitPreview = useCallback(() => {
        ...
        if (preview) {
            ...
            const isBoundaryPreview = !!previewType && isUnifiedBoundaryType(previewType);

            if (isBoundaryPreview) {
                preview.points([]);
                preview.closed(false);
                preview.visible(false);
            } else {
                const rawPreviewPoints = [lastX, lastY, fallbackNext.x, fallbackNext.y];
                preview.points(rawPreviewPoints);
                preview.closed(false);
    ...
    const handleMouseDown = useCallback(
        (e: any) => {
            ...
            if (evt.button === 0 && mode === "draw" && (activeTool === "draw" || activeTool === "cut")) {
                ...
                // ✅ Boundaries: punten zetten, maar wél kunnen sluiten op het beginpunt
                if (activeDrawType && isUnifiedBoundaryType(activeDrawType)) {
                    const base = draftPointsRef.current;
                ...
                const lineOnly = !!activeDrawType && isUnifiedBoundaryType(activeDrawType);
    ...
    const handleMouseMove = useCallback(
        (e: any) => {
            ...
            if (isEdgeResizingRef.current && activeTool === "select") {
                ...
                if (line) {
                    const liveRenderPoints =
                        editedObj && isUnifiedBoundaryType(editedObj.type)
                            ? getBoundaryBandPoints(nextPoints, editedObj.type) ?? nextPoints
                            : nextPoints;

                    line.points(liveRenderPoints);
            ...
            if (isVertexDraggingRef.current && activeTool === "select") {
                ...
                const line = selectedLineRefs.current[edit.objectId];
                if (line && edit.holeIndex === null && editedObj && !isUnifiedBoundaryType(editedObj.type)) {
    ...
    useEffect(() => {
        ...
        if (isRestoringRightStepMenuRef.current) {
            isRestoringRightStepMenuRef.current = false;
            return;
        }
